...
### Publish client
- [x] Thin shared `pymergetic-metal-cdn-client` (`pymergetic.metal.cdn_client`)
- [x] Auth prepared for OAuth (`token_source`; Bearer-only contract)
- [x] pack → AOT → sign → zlib → upload one-shot (`wasmmod.py publish`)
- [x] GitHub Action (metal-cdn reusable upload + wasmmod one-shot workflow)
- [x] Web upload UI (`GET /publish`)
- [x] Admin trust roots + `METAL_CDN_REQUIRE_SIGNED` (`off`/`present`/`verify`)
...
### Storage / edge
- [x] S3/MinIO backend (`METAL_CDN_STORAGE_BACKEND`)
...
- [x] Optional signed index (`METAL_CDN_INDEX_SIGNING_KEY`)

### Ops
- [x] Alembic migrations (`metal-cdn db upgrade`, `alembic/`)
- [x] Structured logs / metrics (`METAL_CDN_JSON_LOGS`, `GET /metrics`)
...
- [x] Dist `pymergetic-metal-cdn` / import `pymergetic.metal.cdn` (PEP 420 `pymergetic.metal`)

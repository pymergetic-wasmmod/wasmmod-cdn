...
from pymergetic.wasmmod.cdn.security import (
    generate_api_key,
    hash_api_key,
    hash_password,
    verify_password,
...
class ApiKeyService:
    ...
    async def resolve(self, full_key: str) -> tuple[UUID, list[str], UUID] | None:
        """Return ``(user_id, scopes, api_key_id)`` for a live API key, or ``None``."""
        digest = hash_api_key(full_key)
        result = await self._session.exec(select(ApiKey).where(ApiKey.key_hash == digest))
        row = result.first()
        if row is None or row.revoked_at is not None:
            return None
        return row.user_id, scopes_from_storage(row.scopes), row.id
...
class AclService:
    ...
    async def is_unclaimed(self, package_name: str) -> bool:
        ...
        if result.first() is not None:
            return False
        from pymergetic.wasmmod.cdn.models import PackageTeamAcl

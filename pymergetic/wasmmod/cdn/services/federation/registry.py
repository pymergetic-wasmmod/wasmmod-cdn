...
from pymergetic.wasmmod.cdn.services.federation.prefix import normalize_mount_prefix
from pymergetic.wasmmod.cdn.services.federation.registry_grants import (
    FEDERATION_BOT_EMAIL,
    GrantOpsMixin,
)
from pymergetic.wasmmod.cdn.services.federation.registry_mounts import MountOpsMixin
from pymergetic.wasmmod.cdn.services.federation.registry_peers import PeerOpsMixin
...
class FederationRegistry(PeerOpsMixin, MountOpsMixin, GrantOpsMixin):
    ...
    async def status(self) -> FederationStatus:
        ...
        return FederationStatus(
            ...
            proxy_ready=True,
            detail="read proxy + PUSH publish; Bearer or MetalFed tickets",
        )

    def parse_bootstrap_mounts(self, raw: str | None) -> list[dict]:
        """Parse ``WASMMOD_CDN_FEDERATION_MOUNTS_JSON`` (list of dicts)."""

...
class S3ObjectStorage(ObjectStorage):
    ...
    async def _client(self) -> AsyncIterator[Any]:
        try:
            ...
            raise RuntimeError(
                "S3 backend requires aioboto3 — pip install 'pymergetic-wasmmod-cdn[s3]'"
            ) from exc
...
def build_storage(settings: Settings) -> ObjectStorage:
    ...
    if backend in ("s3", "minio"):
        if not settings.s3_bucket or not settings.s3_access_key or not settings.s3_secret_key:
            raise ValueError("S3 backend requires WASMMOD_CDN_S3_BUCKET / ACCESS_KEY / SECRET_KEY")
...
async def collect_orphan_keys(storage: ObjectStorage) -> list[str]:
    ...
    for key in keys:
        if key == "index.json" or key.endswith("/index.json"):
            ...
            from pymergetic.wasmmod.cdn.models import ChannelIndex

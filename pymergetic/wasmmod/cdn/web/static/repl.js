...
(() => {
  ...
  const resetBtn = document.getElementById("mpy-repl-reset");
  ...
  const HEAP_STORE_PREFIX = "wasmmod-cdn.mpy.heapsize.";
  ...
  function syncDeadUi() {
    const e = cur();
    const dead = !!(e && e.dead);
    if (resetBtn) resetBtn.hidden = !dead;
    if (inEl) inEl.disabled = dead;
    if (dead) setStatus("dead", e);
  }

  function markDead(eng, reason) {
    const e = eng || cur();
    if (!e) return;
    e.dead = true;
    e.sessionReady = false;
    append(reason || "seat shut down — Reset to boot again", "mpy-err", e);
    syncDeadUi();
  }
  ...
  async function run(code, { echo = true, bootstrap = false, quiet = false } = {}) {
    ...
    if (!e) return;
    if (e.dead && !bootstrap) {
      append("seat dead — press Reset to boot again", "mpy-err", e);
      return;
    }
    ...
    try {
      ...
      // After shutdown()/unboot dead face: boot.is_dead() if nest present.
      if (!bootstrap && typeof runtime.runPython === "function") {
        try {
          const dead = runtime.runPython(
            "bool(getattr(__import__('pymergetic.metal.boot',None,None,('is_dead',)),'is_dead',lambda:False)())",
          );
          if (dead) markDead(e, "shutdown complete — seat dead");
        } catch (_) {
          /* nest may be absent on vanilla upy */
        }
      }
    } catch (err) {
      const msg =
      ...
      if (/SystemExit|seat dead|shut down/i.test(msg)) {
        markDead(e);
      }
    }
  }
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", () => {
      ...
      append("Reset — booting seat again", "mpy-out", e);
      void ensureSession({ quiet: false }).catch(() => {});
    });
  }

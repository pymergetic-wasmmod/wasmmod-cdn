...
from pymergetic.wasmmod.cdn.paths import normalize_base_path, path_prefix
from pymergetic.wasmmod.cdn_client.verify import RequireSignedMode


class Settings(BaseSettings):
    """Runtime configuration for wasmmod-cdn."""

    model_config = SettingsConfigDict(
        env_prefix="WASMMOD_CDN_",
        env_file=".env",
        env_file_encoding="utf-8",
        extra="ignore",
    )

    app_name: str = "wasmmod-cdn"
    debug: bool = False
    data_dir: Path = Field(default=Path(".data"))
    storage_root: Path = Field(default=Path(".data/packs"))
    database_url: str = Field(
        default="sqlite+aiosqlite:///.data/wasmmod_cdn.db",
    ...
    session_secret: str = Field(
        default="dev-change-me-wasmmod-cdn",
    ...
    experimental: bool = Field(
        ...
        description=(
            ...
            "(WASMMOD_CDN_EXPERIMENTAL=0 to disable after go-live)"
        ),
    )
    ...
    brand_name: str = Field(
        default="",
        description=(
            "Header product label. Empty → use app_name (default wasmmod-cdn). "
    ...
    def display_brand_name(self) -> str:
        return (self.brand_name or self.app_name or "wasmmod-cdn").strip() or "wasmmod-cdn"

"""HTTP client for wasmmod-cdn (stdlib urllib only)."""
...
from pymergetic.wasmmod.cdn_client.client_inspect import ArtifactInspectMixin
from pymergetic.wasmmod.cdn_client.config import load_config
from pymergetic.wasmmod.cdn_client.http_util import (
    ArtifactDownload,
    ClientError,
    _encode_multipart,
    _header,
...
class CdnClient(ArtifactInspectMixin):
    ...
    def from_config(cls, *, timeout: float = 60.0, require_token: bool = False) -> CdnClient:
        ...
        token = cfg.get("token")
        if not url:
            raise ClientError("CDN URL not set — wasmmod-cdn login --url … or config url")
        if require_token and not token:
            raise ClientError("not logged in — set token (e.g. wasmmod-cdn login --url …)")

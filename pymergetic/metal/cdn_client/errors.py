"""User-facing CLI error formatting for metal-cdn / wasmmod tools.

Keep messages short, on stderr via :func:`report`, with optional indented hints.
Set ``METAL_CDN_DEBUG=1`` (or ``WASMMOD_DEBUG=1``) to re-raise unexpected errors
...
from pymergetic.metal.cdn_client.client import ClientError
...
def hints_for_client_error(exc: ClientError, *, force: bool = False) -> list[str]:
    ...
    if status == 409 and not force:
        ...
        hints.append("Check credentials: metal-cdn login --url …  (or --token / METAL_CDN_TOKEN)")
        hints.append("Verify with: metal-cdn whoami")
    elif status == 404:
        ...
        hints.append("Is the CDN up? (e.g. metal-cdn serve)")
        hints.append("Check --cdn-url / METAL_CDN_URL / metal-cdn login config")
    elif status is None and ("not set" in msg or "not logged in" in msg):
        hints.append("metal-cdn login --url http://127.0.0.1:8000/cdn --email … --register")
...
def _debug_enabled() -> bool:
    for key in ("METAL_CDN_DEBUG", "WASMMOD_DEBUG"):
...
def invoke(main_fn, *, prog: str = "metal-cdn") -> int:
    ...
    try:
        ...
        print(
            format_error(
                ...
                "Set METAL_CDN_DEBUG=1 or WASMMOD_DEBUG=1 for a traceback",
            ),
            file=sys.stderr,
        )
        return 1
